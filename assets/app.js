...
async function initFirebase() {
  ...
  try {
    ...
    // Read the current DB state once. If the DB is empty, seed it with the
    // local starter data (tasks/team) so the whole team starts with content.
    const snap = await get(fbRef);
    if (!snap.exists() || !snap.val() || !snap.val().tasks) {
      await set(fbRef, {tasks, team, dark, appTitle, collapsed, log});
    }

    fbReady = true;

    // Real-time listener for subsequent changes from any user.
...
async function load() {
  // Render immediately with the built-in starter data (or localStorage) so the
  // UI is never blank, even if Firebase is slow or unreachable.
  try {
  ...
  // Then connect to Firebase in the background. If it succeeds, the onValue
  // listener refreshes the data live; if it fails, we simply stay on local data.
  try {
    await initFirebase();
  } catch (e) {
    console.error('initFirebase threw:', e);
    showSync('error', 'Sem sincronização');
  }
}
...
//  This file is loaded as a <script type="module">, so its top-level
//  functions are module-scoped and NOT visible to inline HTML handlers
//  like onclick="openModal()". We expose them explicitly on window so the
//  inline handlers in index.html (and in rendered card templates) work.
// ═══════════════════════════════════════════════════
Object.assign(window, {
  // header / view controls
  toggleHide, toggleTheme, toggleSearch,
  toggleCatOpen, toggleRespOpen,
  setCat, setResp, toggleSection,
  startTitleEdit, saveTitleEdit,
  // task modal
  openModal, closeTask, saveTask,
  toggleDrop, togResp, rmResp,
  setStatus, delTask,
  // team modal
  openTeam, closeTeam, addMember, saveTeam,
  // avatar editor
  openAvEdit, openAvForName, openAvForNameFromProfile,
  closeAv, setAvColor, removeAvSvg, handleSvg, saveAv,
  // profiles
  openProfiles, closeProfiles,
  // workload
  openWorkload, closeWorkload,
  // log
  ...
  // render (used by oninput on search field)
  render, persist,
});
